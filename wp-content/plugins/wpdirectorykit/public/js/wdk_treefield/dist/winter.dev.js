...
jQuery.fn.wdkTreefield = function (options) {
  ...
  */
  ...
  return this.each(function () {
    ...
    options.obj.parent().find(".circle-loading-bar").addClass(options.progressBar); // open scroll part
    ...
    options.obj.parent().find('.btn-group:first button').on('click', function () {
      ...
      if (container.hasClass('win_visible')) {
        ...
        if (options.firstLoad) options.obj.parent().find('.list_scroll').scrollTop(0);
  ...
  function loadMore() {
    ...
    if (options.offset == 0) options.obj.parent().find('ul').html('');
    var param = {
    ...
    jqxhr = jQuery.post(options.ajax_url, param, function (data) {
      ...
      if (data.success == false) {
        ...
        if (options.offset == 0) options.obj.parent().find('.list_scroll').scrollTop(0);
  ...
  function generateHtml() {
    ...
    options.obj.before('<div class="wdk_dropdown_tree">' + // showing value, always visible
    '<div class="btn-group ">' + '<button class="btn btn-default" type="button">' + options.currValue + '&nbsp;' + '</button>' + '<button type="button" class="btn btn-default dropdown-toggle"> <i class="fa fa-angle-down"></i> </button>' + '</div>' + // hidden part with scroll and search
    ...
    '<div class="input-group">' + '<input type="text" class="form-control search_term" placeholder="' + options.text_search + '" aria-describedby="basic-addon2"  autocomplete="off">' + '<span class="input-group-addon"><i class="loader-spiner fa fa-spinner fa-spin"></i></span>' + '</div>' + '</div>' + '</div>');
  }
};